    ...
    async def check_purchased_the_user(review_data: ReviewCreateSchema):
        try:
            async with aiohttp.ClientSession() as session:
                url = f"http://order_service:8002/orders/users/{review_data.user_id}/purchased-products/{review_data.product_id}"
...
async def create_review(
    session: AsyncSession, review_data: ReviewCreateSchema
) -> dict:
    # if not await ReviewUtils.check_purchased_the_user(review_data):
    #     raise HTTPException(
    #         status_code=status.HTTP_403_FORBIDDEN,
    #         detail="User did not purchase this product",
    #     )
    ...
    new_rating = await ReviewUtils.get_avg_rating(session, review_data)
    try:
...
async def get_review_by_id(session: AsyncSession, review_id: int) -> ReviewModel:
    stmt = (
        select(ReviewModel)
...
async def update_review(
    session: AsyncSession, review_data: ReviewUpdateSchema, review_id: int
):
